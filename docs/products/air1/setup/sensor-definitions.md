...
=== "Configuration"
    ...
    | **CO2 Auto Calibration** | On | Keeps the SCD40 CO₂ sensor calibrated automatically. The sensor assumes it sees fresh air (about <a href="https://climate.nasa.gov/vital-signs/carbon-dioxide/?intent=121" target="_blank" rel="noopener">420 ppm</a>) at least once a week and corrects its baseline to match. Turn it off if the device sits in a space that rarely gets fresh air, then <a href="https://wiki.apolloautomation.com/products/general/calibrating-and-updating/co2-calibration/" target="_blank" rel="noopener">calibrate manually</a> with **Calibrate SCD40 To 420ppm** every 1 to 2 years. |
    ...
    | **Air Quality LED Source** | Off | Colors the three onboard RGB LEDs by air quality. Choose **NowCast AQI**, **CO2**, or **VOC Index** and the LEDs shift from green to red as that reading climbs. Leave it on **Off** to keep the LEDs free for your own automations. See the <a href="https://wiki.apolloautomation.com/products/air1/additional-info/led-indicator/" target="_blank" rel="noopener">LED Indicator guide</a>. |
    | **Air Quality LED Brightness** | 100 % | Dims the air-quality LEDs, from 5% to 100%. Applies the moment you change it. Only has an effect while **Air Quality LED Source** is set to something other than **Off**. |
    | **Bluetooth Proxy** | Off | Turns the AIR-1 into a <a href="https://wiki.apolloautomation.com/products/air1/additional-info/bluetooth-proxy/" target="_blank" rel="noopener">Bluetooth proxy</a> so nearby Bluetooth devices reach Home Assistant through it. Toggles on without reflashing. |
    | **Firmware Channel** | Stable | Sets which firmware the **Firmware Update** entity pulls. **Stable** is the tested release; **Beta** gets new features earlier. Switch back to **Stable** any time to return to the stable build. |
